...
def filter_terms(inFile, vocab):
    ...
    with open(inFile) as f:
        for line in f.readlines():
            ...
            source = tokens[0].replace(' ','_').strip().lower()
            target = tokens[1].replace(' ','_').strip().lower()
...
def build_analogy(rels, outFile, number=10000):
    ...
    pairs_single_end = [x for x in pairs if x[1][1].count('_') == 0]
...
def main():
    ...
    if len(sys.argv) <= 3:
        print('Usage: [vocab_files] [input_files] [output_file]')
